...
3. **근거 확실성** — 후보집합 8/8, 공급지표 순위상관 0.931, 구성요소 제거 시 후보 교체와 이동시간 가정 범위를 구분
...
| 마을버스 1차 스크리닝 | 경계 안 2,095개 중 1,632개 정류장에 마을노선 표기 | 배차·운행량·목적지 중복은 미확인 |
| 의료시설 이동시간 가정 | 후보별 대기 5·10·15분의 30·45분 면적커버리지·시간 범위 | 실제 도입 전후·대상자·62개 서비스 효과가 아닌 공개 가정 시나리오 |
| 복지 목적지 준비 | 공식 경로당 웹 593건·Excel 594행, 노인종합복지관 3곳 | 좌표 0건이므로 접근성 계산 전; Juso 별도 승인키 필요 |
...
공식 목적지 보완에서는 관산동 경로당 `16곳`을 포함해 Excel 594행을 44개 행정동에 집계했습니다. 다만 현재 웹 총계는 593건이며 좌표가 없으므로, 목록 수를 접근성이나 실제 통합돌봄 서비스 이용 가능성으로 해석하지 않습니다.
