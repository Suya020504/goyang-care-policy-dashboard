...
1. **판단 질문** — 사업 도입지가 아니라 현장조사 대상으로 올릴지 범위를 고정
2. **행주동 신호** — 고령화율·버스 연결·의료시설 거리 세 신호만 도시 기준과 비교
3. **근거 확실성** — 후보집합 8/8, 제한 가중치 45/45, 전체 경계감사 206/231을 구분
4. **확인할 빈칸** — 공개데이터에 없는 실제 목적지·호출·승하차·운영자료를 체크
5. **대안별 조사** — 다섯 질문을 한 문항씩 확인하며 답변은 미확인·예·아니오로만 기록
6. **체크리스트 저장** — 자유입력란 없이 개인정보 없는 JSON 파일로 구조화된 현장조사 항목 저장
...
서버 전송과 사용자 추적은 없습니다. 안내형 화면의 진행 단계·체크 항목·대안 조사 답변과 분석 상세의 검토 상태는 현재 브라우저의 `localStorage`에만 저장됩니다. 안내형 저장 파일에는 자유입력란이 없고 구조화된 현장조사 항목만 포함됩니다. 공용 PC에서는 분석 상세의 사유 입력란에 개인정보를 적지 말고 시연 후 초기화하세요. 자세한 내용은 [PRIVACY.md](PRIVACY.md)를 확인하세요.
